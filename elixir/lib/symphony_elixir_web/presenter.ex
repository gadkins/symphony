defmodule SymphonyElixirWeb.Presenter do
  ...
  def issue_payload(issue_identifier, orchestrator, snapshot_timeout_ms) when is_binary(issue_identifier) do
    case Orchestrator.snapshot(orchestrator, snapshot_timeout_ms) do
      %{} = snapshot ->
        ...
        parked = ParkedRuns.get(issue_identifier)

        if is_nil(running) and is_nil(retry) and is_nil(blocked) and is_nil(parked) do
          {:error, :issue_not_found}
        else
          {:ok, issue_payload_body(issue_identifier, running, retry, blocked, parked)}
        end
  ...
  defp issue_payload_body(issue_identifier, running, retry, blocked, parked) do
    %{
  ...
  defp logs_payload(issue_identifier, running, blocked, parked, retry \\ nil) do
    ...
    %{
      ...
      engine_match_key: "issue_identifier=#{issue_identifier}",
      codex_path: codex_path,
      engine_tail: EngineLogTailer.initial_lines(issue_identifier, max_lines: @log_tail_max_lines),
      codex_tail: codex_tail
    }
  end

  defp codex_logs(session_id) do
    opts = codex_sessions_opts()

    case CodexSessionTailer.resolve_path(session_id, opts) do
      {:ok, path} ->
        tail =
          path
          |> CodexSessionTailer.readable_lines(opts)
